/**
 * EKHTYARATY - Worker رئيسي (Static Assets + API)
 * =================================================
...
// ===== عداد الزيارات (D1) =====
// بيسجّل زيارة واحدة لكل يوم في جدول visits_daily. لو الـ D1 binding (VISITS_DB) لسه مش مضاف
// من الداشبورد، بيتجاهل بهدوء من غير ما يعطّل تحميل الموقع خالص.
  ...
  try {
    ...
    await env.VISITS_DB.batch([
      env.VISITS_DB.prepare(
        'CREATE TABLE IF NOT EXISTS visits_daily (day TEXT PRIMARY KEY, count INTEGER NOT NULL DEFAULT 0)'
      ),
      env.VISITS_DB
        .prepare('INSERT INTO visits_daily (day, count) VALUES (?, 1) ON CONFLICT(day) DO UPDATE SET count = count + 1')
        .bind(day),
    ]);
  } catch (e) {
    console.error('recordVisit error:', e && e.message ? e.message : String(e));
  }
}
...
async function handleRequest(request, env, ctx) {
    ...
    // ===== إحصائية الزيارات: الإجمالي + النهاردة + آخر 7 أيام (محمي بتوكن الأدمن) =====
    if (url.pathname === '/api/visits' && request.method === 'GET') {
      ...
      if (!env.VISITS_DB) {
        return json({ ok: true, enabled: false, total: 0, today: 0, last7: [] });
      }
      try {
        const today = new Date().toISOString().slice(0, 10);
        const [totalRow, todayRow, last7Res] = await Promise.all([
          env.VISITS_DB.prepare('SELECT COALESCE(SUM(count), 0) AS total FROM visits_daily').first(),
          env.VISITS_DB.prepare('SELECT count FROM visits_daily WHERE day = ?').bind(today).first(),
          env.VISITS_DB.prepare('SELECT day, count FROM visits_daily ORDER BY day DESC LIMIT 7').all(),
        ]);
        const last7 = (last7Res && last7Res.results ? last7Res.results : []).reverse();
        return json({
          ok: true,
          enabled: true,
        ...
        // لسه مفيش جدول (يعني لسه مفيش زيارة اتسجلت من الأساس) - نرجع أصفار بدل ما نطلع خطأ
        return json({ ok: true, enabled: true, total: 0, today: 0, last7: [] });
      }
    }
    ...
    if (isSpaRoute) {
      // بنسجل الزيارة في الخلفية (من غير ما نستنى النتيجة) عشان مايبطأش تحميل الصفحة للزائر خالص
      if (request.method === 'GET') ctx.waitUntil(recordVisit(env));
      ...
      const descriptionToUse = product
        ? `${product.name} - ${product.price} ج.م` + (product.oldPrice ? ` (بدل ${product.oldPrice} ج.م)` : '') + ' | EKHTYARATY'
        : citem
          ? `${citem.name} - ${citem.price} ج.م` + (citem.oldPrice ? ` (بدل ${citem.oldPrice} ج.م)` : '') + (citem.promoText ? ` | ${citem.promoText}` : '') + ' | EKHTYARATY'
          : null;
      ...
      class TitleRewriter {
        element(el) {
          if (product) el.setInnerContent(`${product.name} | EKHTYARATY`);
          else if (citem) el.setInnerContent(`${citem.name} | EKHTYARATY`);
        }
      }
